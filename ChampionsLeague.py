...
def _stage_play(stg, teams_for_pairs, all_teams):
    while stg >= 1:
        ...
        for i in range(stg):  # for every 2 teams per stage

            left_side = teams_for_pairs.pop()  # pick random 1st team
            right_side = teams_for_pairs.pop()  # pick random 2nd team
            ...
            if score_left > score_right:
                all_teams.remove(right_side)
            elif score_left < score_right:
                all_teams.remove(left_side)
                ...
                while (pen_score_left == pen_score_right):
                    pen_score_right = random.randint(0, 6)
                file.write("(Penalty: {} - {})\n".format(pen_score_left,
                                                         pen_score_right))
                print("(Penalty: {} - {})".format(pen_score_left,
                                                  pen_score_right))
        ...
        teams_for_pairs.update(all_teams)
        stg //= 2
...
def _show_results():
    ...
    while in_ch == 89 or in_ch == 121:  # (89 == 'y', 121 == 'Y')
        ...
        stg = 8
        key_check = False  # Check for penalties
        for line in file:
            if key_check is True and "Penalty" in line:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if in_ch == 49:  # 1) Play! (49 == '1')
            ...
            _stage_play(stage, team_list, tmp_teams)  # 1/8, 1/4, 1/2, Final
